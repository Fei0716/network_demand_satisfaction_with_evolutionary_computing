...
void printChromosome() {
	...
	for (int c = 0; c < POP_SIZE; c++) {
		cout << "\tChromosome" << c << " ";
		for (int g = 0; g < GENE; g++) {
			tempIndex = chromosome[c][g];
			cout << "\t" << tableData[tempIndex][0];
		}
		cout << endl;
	}
...
void survivalSelection() {
	...
	for (int c = 0; c < POP_SIZE; c++) {
		...
		for (int g = 0; g < GENE; g++)
		{
			int tempIndex = newChromosome[c][g];
...
void recordBestFitness() {
	...
	for (int g = 0; g < GENE; g++) {
		int tempIndex = bestChromo[g];
		cout <<"\t" << tableData[tempIndex][0] << " ";
		bestChromoFile << tableData[tempIndex][0] << " ";
	}
	cout << endl;
	bestChromoFile << endl;
}
